...
export async function authenticate(req: Request, res: Response) {
  ...
  try {
    ...
    const opts: SignOptions = { subject: user.id, expiresIn: env.JWT_EXPIRES_IN } // número (segundos)
    const token = JWT.sign({ email: user.email, name: user.name }, env.JWT_SECRET, opts)
